...
ExecHashJoin(HashJoinState *node)
{
	EState	   *estate;
	PlanState  *outerNode;
	HashState  *outerNode; /* CSI 3130 - Name change */
	HashState  *innerNode; /* CSI 3130 - Added */
	List	   *joinqual;
	List	   *otherqual;
	TupleTableSlot *inntuple;
	TupleTableSlot *outtuple; /* CSI 3130 - Added */
	ExprContext *econtext;
	ExprDoneCond isDone;
	HashJoinTable inner_hashtable; /* CSI 3130 - Name change */
	HashJoinTable outer_hashtable; /* CSI 3130 - Added */
	HashJoinTuple curtuple;
	TupleTableSlot *outerTupleSlot;
	TupleTableSlot *innerTupleSlot; /* CSI 3130 - Added */
	uint32		hashvalue;
	int			batchno;
	...
	innerNode = (HashState *) innerPlanState(node); /* CSI 3130 - Name change hashnode*/
	outerNode = (HashState *) outerPlanState(node); /* CSI 3130 - Added*/
	outerNode = outerPlanState(node);

	/*
	 * get information from HashJoin state
	 */
	inner_hashtable = node->inner_hj_HashTable; /* CSI 3130 - hj inner hash table*/
	outer_hashtable = node->outer_hj_HashTable; /* CSI 3130 - hj outer hash table*/
	 ...
	 */
	 /* CSI 3130 - comment the following paragraph */
	 ...
	 */
	if (inner_hashtable == NULL||outer_hashtable == NULL) /* CSI 3130 change : we know check condition on both hash tables */
	{
		/*
...
/* 		if (HASHJOIN_IS_OUTER(node) ||
		 ...
		 */
		inner_hashtable = ExecHashTableCreate((Hash *) innerNode->ps.plan,
										node->hj_HashOperators); /* CSI 3130 : change name to inner */
		outer_hashtable = ExecHashTableCreate((Hash *) outerNode->ps.plan,
										node->hj_HashOperators); /* CSI 3130 : creation of the second hash table (outer one)*/
		node->hj_HashTable = hashtable;

		/*
		 * execute the Hash node, to build the hash table
		 */
		innerNode->hashtable = inner_hashtable; /* CSI 3130 : change name */
+		outerNode->hashtable = outer_hashtable; /* CSI 3130 : same thing done on the outer hash table */
		 ...
		 */
		 /* CSI 3130 : commenting the following paragraph */
		 ...
		 */
		 /* CSI 3130 : following line commented */
	...
	for (;;)
	{
		 ...
		 */
		 /* CSI 3130 : following paragraph commented */
		...
		// {
			...
			/* we replace the preceding function by calling the ExecProcNode function */
		...
		for (;;)
		{
			...
			if (joinqual == NIL || ExecQual(joinqual, econtext, false))
			{
				...
				if (otherqual == NIL || ExecQual(otherqual, econtext, false))
				{
					...
					{
						node->matches_by_probing_outer=node->matches_by_probing_outer+1; /* CSI 3130 : added */
...
{
	HashJoinState *hjstate;
	Hash	   *outerhashNode; /* CSI 3130 - Name change */
	Hash	   *innerhashNode; /* CSI 3130 - Creation of a hash node for the inner*/
	List	   *lclauses;
	List	   *rclauses;
	List	   *hoperators;
	ListCell   *l;
	 ...
	 */
	outerNode = (Hash *) outerPlan(node); /* CSI 3130 - creating outerNode plan*/
	innerNode = (Hash *) innerPlan(node); /* CSI 3130 - creating innerNode plan*/

	/* CSI 3130 - creating child nodes */
	...
	hjstate->hj_InnerTupleSlot = ExecInitExtraTupleSlot(estate); /* CSI 3130 */
	...
	ExecSetSlotDescriptor(hjstate->hj_InnerTupleSlot,
						  ExecGetResultType(InnerPlanState(hjstate))); /* CSI 3130 */

	/*
	 * initialize hash-specific info
	 */
	hjstate->hj_OuterHashTable = NULL; /* CSI 3130 */
	hjstate->hj_InnerHashTable = NULL; /* CSI 3130 */

	hjstate->hj_FirstOuterTupleSlot = NULL; 
	hjstate->hj_FirstInnerTupleSlot = NULL; /* CSI 3130 */

	hjstate->hj_InnerCurHashValue = 0; /* CSI 3130 */
	hjstate->hj_InnerCurBucketNo = 0; /* CSI 3130 */
	hjstate->hj_InnerCurSkewBucketNo = INVALID_SKEW_BUCKET_NO; /* CSI 3130 */
	hjstate->hj_InnerCurTuple = NULL; /* CSI 3130 */
	 
	hjstate->hj_OuterCurHashValue = 0; /* CSI 3130 */
	hjstate->hj_OuterCurBucketNo = 0; /* CSI 3130 */
	hjstate->hj_OuterCurSkewBucketNo = INVALID_SKEW_BUCKET_NO; /* CSI 3130 */
	hjstate->hj_OuterCurTuple = NULL; /* CSI 3130 */
	
	hjstate->inner_exhausted = false;  /* CSI 3130 */
	hjstate->outer_exhausted = false;  /* CSI 3130 */
	
	hjstate->matches_by_probing_inner = 0; /* CSI 3130 */
	hjstate->matches_by_probing_outer = 0; /* CSI 3130 */
	...
	((HashState *) outerPlanState(hjstate))->hashkeys = lclauses; /* CSI 3130 */

	hjstate->js.ps.ps_OuterTupleSlot = NULL; /* CSI 3130 */
	hjstate->js.ps.ps_InnerTupleSlot = NULL; /* CSI 3130 */
	
	hjstate->js.ps.ps_TupFromTlist = false; 
	
	hjstate->hj_NeedNewOuter = true;
	hjstate->hj_NeedNewInner = true; /* CSI 3130 */

	hjstate->hj_MatchedOuter = false;
	//hjstate->hj_OuterNotEmpty = false; /* CSI 3130 */
	...
	if (curbatch == 0)			/* if it is the first pass */
	{
		...
		while (!TupIsNull(slot))
		{
									 ...
									 hashvalue))
			{
				...
				// hjstate->hj_OuterNotEmpty = true; /* CSI 3130 */
